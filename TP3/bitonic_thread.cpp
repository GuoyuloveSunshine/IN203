...
namespace Bitonic{
  ...
  sort( bool up, std::vector<Obj>& x ){
    int set_val = tem_val++;
    int first = set_val*(x.size()/thread_size);
    int end = (set_val+1)*(x.size()/thread_size)-1;
    _sort(up, x.data()+first, end-first+1);
    return x;
  }  
}
...
int main(){     
    ...
    for(int i = 0; i < thread_size; i++){
        t[i] = std::thread(&Bitonic::sort,&(true,tab));
    }
    for(int i = 0; i < thread_size; i++){
      t[i].join();
    }
    ...
    std::thread t[thread_size];
    for(int i = 0; i < thread_size; i++){
        t[i] = std::thread(&Bitonic::sort,&(true,vtab));
    }
    for(int i = 0; i < thread_size; i++){
      t[i].join();
    }
