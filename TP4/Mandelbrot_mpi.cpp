...
int iterMandelbrot( int maxIter, const Complex& c){
    ...
    if ( c.real*c.real+c.imag*c.imag < 0.0625 ) return maxIter;
    if ( (c.real+1)*(c.real+1)+c.imag*c.imag < 0.0625 ) return maxIter;
...
std::vector<int> computeMandelbrotSet( int W, int H, int maxIter, int it_begin, int it_end, int H_morc){
...
int main(int argc, char *argv[] ){
    ...
    if(H%nbp!=0){
        std::cout <<"Must diviser par " << H << std::endl;
        MPI_Abort(globComm, EXIT_FAILURE);
    }

    int H_morc = H/nbp;
    int it_begin = H_morc*(nbp-rank-1);
    int it_end = H_morc*(nbp-rank);

    std::vector<int> iters(W*H), iters_p;
    std::chrono::time_point<std::chrono::system_clock> start, end;
    start = std::chrono::system_clock::now();
    iters_p = computeMandelbrotSet( W, H, maxIter, it_begin, it_end,H_morc);
    
    // Normalement, pour un bon rendu, il faudrait le nombre d'itérations
    // ci--dessous :
    MPI_Gather(&iters_p[0], W*H_morc, MPI_INT, &iters[0], W*H_morc, MPI_INT, 0, globComm);
    if(rank == 0){
        
        std::cout << "je suis [0], et je garde la figure.";
        savePicture("mandelbrot_mpi.tga", W, H, iters, maxIter);
