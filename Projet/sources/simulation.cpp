...
void simulation(bool affiche){
    ...
    std::ofstream output("Courbe_origin.dat");
    output << "# jours_ecoules \t nombreTotalContaminesGrippe \t nombreTotalContaminesAgentPathogene()" << std::endl;
    //Mesurer le temps et le noter
    std::ofstream output_temp("temps_init.dat");
    output_temp << "Jours_ecoules \t Temps_sans_affichage \t Temps_avec_affichage \t Temps_affichage" << std::endl;
    end1 = std::chrono::system_clock::now();
    std::chrono::duration<double> elapsed_seconds = end1-start;
    std::cout << "Temps de l'initialisation : " << elapsed_seconds.count() << std::endl;
    ...
    while (!quitting){
        ...
        // Mesure du temps sans affichage
        end1 = std::chrono::system_clock::now();
        std::chrono::duration<double> temps_sans_affichage = end1-start;
        ...
        std::chrono::duration<double> temps_avec_affichage = end2-start;

        output_temp << jours_ecoules << "\t" << temps_sans_affichage.count() << "\t" 
                    << temps_avec_affichage.count() << "\t" 
                    << temps_avec_affichage.count()-temps_sans_affichage.count() << std::endl;
        jours_ecoules += 1;
